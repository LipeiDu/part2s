...
void eventsTreatment(int nev, double tauform, int *Nbtot, int *Npart){
    ...
    FILE *allsetfile;
    char allsetname[255];
    sprintf(allsetname, "%s.dat", "output/FormedParticles_allEvents"); // All set.dat will be written in this file after recentering and rotation
    ...
    for (int iev=1; iev<nev+1; ++iev)
    {
...
#else
        sprintf(eventname, "input/event_test_kernel.dat");
#endif
        ...
        FILE *milnefile;
        char milnename[255];
        sprintf(milnename, "%s%d.dat", "output/FormedParticles", iev);
        ...
        *Npart = *Npart + NpartEv;
        ...
        if(milnefile==NULL){
            printf("FormedParticles%d.dat could not be opened...\n", iev);
        }
        else{
            ...
            {
                // read in particle info, x in Cartesian, p in Cartesian
...
void testfileTreatment(int nev, double tauform, int *Npart){
    ...
    sprintf(filname, "output/FormedParticles_allEvents.dat");
...
void readInFormedParticles(int Npart, float *r0, float *r1, float *r2, float *r3, float *p0, float *p1, float *p2, float *p3, float *mi, float *gi, float *bi){
    ...
    FILE *allsetfile;
    char allsetname[255];
    sprintf(allsetname, "%s.dat", "output/FormedParticles_allEvents");
    allsetfile = fopen(allsetname, "r");
    
    if (allsetfile == NULL)
        printf("The particle list in FormedParticles_allEvents.dat couldn't be opened...\n");
    else
    {
