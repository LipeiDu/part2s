...
//     {
        ...
        float erfExp = 0.5 * ( erf(-distTem * d_tauInv / 1.41421) + 1 ); // -distTem is to get -d0 = ri0 - t;
...
float KernelMilne(float r1, float r2, float r3, float rm1, float rm2, float rm3, float sigma, float sigman, float SigInv, float SignInv){
...
void source_kernel(int Npart, float tau, float *p0_d, float *p1_d, float *p2_d, float *p3_d, float *r0_d, float *r1_d, float *r2_d, float *r3_d, float *mi_d, float *gi_d, float *bi_d, float *Sb_d, float *St_d, float *Sx_d, float *Sy_d, float *Sn_d, float **stressTensor, float **baryonCurrent, parameters params)
{
    ...
    for (int k = 0; k < Nn; ++k)
    {
        for (int j = 0; j < Ny; ++j)
        {
            for (int i = 0; i < Nx; ++i)
            {
...
#ifdef INITIAL_TENSOR
                stressTensor[0][cid] = facTensor * Ttt; // facTensor = [1/fm^3]
                stressTensor[1][cid] = facTensor * Ttx; // [1/fm^4] = [1/fm^3] * [1/fm]
                stressTensor[2][cid] = facTensor * Tty;
                stressTensor[3][cid] = facTensor * Ttn;
                stressTensor[4][cid] = facTensor * Txx;
                stressTensor[5][cid] = facTensor * Txy;
                stressTensor[6][cid] = facTensor * Txn;
                stressTensor[7][cid] = facTensor * Tyy;
                stressTensor[8][cid] = facTensor * Tyn;
                stressTensor[9][cid] = facTensor * Tnn;
                
                baryonCurrent[0][cid] = facTensor * Nt;
                baryonCurrent[1][cid] = facTensor * Nx;
                baryonCurrent[2][cid] = facTensor * Ny;
                baryonCurrent[3][cid] = facTensor * Nn;
#endif
              ...
              stressTensor[0][cid] = facTensorMilne * Ttt;
              stressTensor[1][cid] = facTensorMilne * Ttx;
              stressTensor[2][cid] = facTensorMilne * Tty;
              stressTensor[3][cid] = facTensorMilne * Ttn;
              stressTensor[4][cid] = facTensorMilne * Txx;
              stressTensor[5][cid] = facTensorMilne * Txy;
              stressTensor[6][cid] = facTensorMilne * Txn;
              stressTensor[7][cid] = facTensorMilne * Tyy;
              stressTensor[8][cid] = facTensorMilne * Tyn;
              stressTensor[9][cid] = facTensorMilne * Tnn;
#endif
            }
        }
    }
